		...
		if ($amount > 0) {
			...
			if ($rs=="" or mysqli_num_rows($rs)==0) {
				...
				if ($gap >= 0) {
					$gID = ggPH($user_id,$amount,$plan);
					ggAccessLog1($user->username,"PH","$gID $amount $plan");
					echo json_encode(array('status'=>"success",'msg'=>$amount));
				} else {
					echo json_encode(array('status'=>'fail','msg'=> "1",'gap'=>-$gap));
				}
			}
		} else {
			echo json_encode(array('status'=>'fail','msg'=> "2"));
		}
	} else {
			echo json_encode(array('status'=>'fail','msg'=> "申請提供幫助已達上限"));
	}
} else {
	echo json_encode(array('status'=>'fail','msg'=> "2"));
}
