...
function doActivate() {
  global $db,$req,$user,$debug;
  $user_id = $req['user_id'];


  $pin = ggPinCount(1);
  $user_id_c = ggFetchValue("select id from tblmember where id=$user_id and pin=''");

  $v = new FormValidator();
  $v->addValidation(1,$pin,"gt=0","激活码不足");
  $v->addValidation(2,$user_id,"eq=".$user_id_c,"找不到这个要激活的用户");

  if (!$v->ValidateForm()) {
    $ret = array("status"=>"fail", "msg"=>$v->getError());
  } else {
    $pin = ggFetchValue("select pin from tblpin1 where managerid = $user->id and status='N' limit 1");
    $rs1 = $db->query("update tblmember set pin='".$pin."' where id=$user_id") or die("Err ".$db->error);
